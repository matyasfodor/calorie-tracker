  ...
  return (
    <div>
      <EntryModal
        ...
        onSubmit={async (entry: FoodEntryWithoutId) => {
          await updateFoodEntry({ variables: { entry, entryId: props.record.id } })
        }}
        entry={props.record}
      />
...
export const AdminEntries = () => {
  ...
  const { loading, data } = useGetAllEntries({
  ...
  const footerRender = () => (
    <EntryModal
      ...
      onSubmit={(entry) => {
        ...
        void createFoodEntry({ variables: { entry: entryWithoutOwner, ownerId } })
      }}
    />
  )
